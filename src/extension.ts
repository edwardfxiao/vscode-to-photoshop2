...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log('vscode-to-photoshop2 is now active');
  let outputChannel = vscode.window.createOutputChannel('Extend Script');
  let disposable = vscode.commands.registerCommand('extension.evaluateScript2', () => {
    ...
    fs.writeFile(pathTojsx, text, (err) => {
      ...
      if (process.platform === WINDOWS) {
        let script = `wscript ${path.resolve(__dirname, '../../photoshop.vbs')} ${pathTojsx} "${JSON.stringify({ filename: `'${editor.document.fileName}'` })}"`;
        cs.exec(script, cb);
      }
    ...
    function cb(err: any, res: any, raw: any) {
      if (err) { return showOutput(`error: ${err.message} \n stack: ${err.stack}`); };
      showOutput(res);
    }
